...
- [ ] Fluxo completo: aluno → instrutor → booking → pagamento → aula → avaliação. (booking/pagamento mock ok; falta concluir aula + avaliação)
...
## Backlog Técnico (Dev)
### Frontend – Navegação
- [x] Corrigir navegação de botões/links no frontend (evitar elementos interativos aninhados).
...
- [x] `/api/auth/user` compatível com modo local/OIDC e retorna `instructorProfile`.
- [x] Login local com email/senha e seed de usuários de teste (admin/instructor/student).
...
- [ ] Checkout/pagamento: alinhar com backend e atualizar status do booking. (gateway real exige API key; falta polling/status real quando há `paymentUrl`)
